    ...
    return {
    	node(nodeLabel) {
    		stage("[$prefix] Clone") {
	    		...
	    		checkout scm
                if (isWindows) {
                    env.BUILD_QUALIFIER = bat(returnStdout: true, script: '@set TZ=UTC&& git show -s --format=%%cd-%%h --date=format-local:%%Y%%m%%d%%H%%M%%S HEAD').trim()
                } else {
                    env.BUILD_QUALIFIER = sh(returnStdout: true, script: 'TZ=UTC git show -s --format=%cd-%h --date=format-local:%Y%m%d%H%M%S HEAD').trim()
                }
